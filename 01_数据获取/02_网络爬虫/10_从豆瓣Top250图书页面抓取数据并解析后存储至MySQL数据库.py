...
def insert_data(data_list):
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', password=' nhcwc', db='spider',
                           charset='utf8')  # 连接数据库
...
# 该Python脚本从豆瓣Top250图书页面抓取数据并解析后存储至MySQL数据库。

# get_html(url)：接收URL参数，获取网页HTML内容并返回。
# ana_by_bs4(html)：解析HTML内容，提取图书信息（如图片链接、书名、作者等），并以列表形式返回。
# insert_data(data_list)：将提取的数据插入到名为spider的MySQL数据库的books表中。
# 主函数循环遍历10页图书列表，依次调用上述函数处理每页数据。
